...
function buildBreadcrumb(pathname: string) {
  ...
  // Remove root if it duplicates the first real crumb
...
export function Header({ user, ncCount }: HeaderProps) {
  ...
  // Ctrl+K listener delegado ao BuscaGlobal, mas o botão também pode abrir
  useEffect(() => {
  ...
  return (
    <>
      ...
      <header className="h-16 bg-white border-b border-[#D5E3F0] flex items-center justify-between px-4 md:px-6 sticky top-0 z-30 shadow-[0_1px_3px_rgba(14,46,96,0.06)] shrink-0">
      {/* Esquerda: hamburguer + breadcrumb */}
      <div className="flex items-center gap-3 min-w-0">
        ...
        <span className="sm:hidden font-semibold text-[#0E2E60] text-sm truncate">
          {breadcrumb[breadcrumb.length - 1]?.label ?? 'MerendaCheck'}
        </span>
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        {/* Avatar */}
        <DropdownMenu>
          <DropdownMenuTrigger className="flex items-center gap-2 pl-2 ml-1 border-l border-[#D5E3F0] hover:opacity-80 transition-opacity">
          ...
          <DropdownMenuContent align="end" className="w-52">
            ...
            <DropdownMenuItem className="flex items-center gap-2 cursor-pointer">
              <User size={14} />
              <Link href="/perfil" className="flex-1">Meu Perfil</Link>
            </DropdownMenuItem>
            <DropdownMenuSeparator />
